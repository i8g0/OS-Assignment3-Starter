...
class SharedResources {
    ...
    // Binary Semaphore to limit concurrent CPU access to 1 process at a time
...
class Process implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            // Release CPU semaphore here to allow other processes to run
    ...
    public void runToCompletion() {
        // Acquire CPU semaphore before running to completion
        try {
            SharedResources.cpuSemaphore.acquire();
        } catch (InterruptedException e) {
            System.out.println(Colors.RED + "  ✗ " + name + " was interrupted while waiting for CPU." + Colors.RESET);
        ...
        try {
            ...
            // Release CPU semaphore to allow other processes to run
            SharedResources.cpuSemaphore.release();
        }
    }
...
public class SchedulerSimulationSync {
    public static void main(String[] args) {
        ...
        int studentID = 445052806;  // ← Student ID set to user's actual ID
